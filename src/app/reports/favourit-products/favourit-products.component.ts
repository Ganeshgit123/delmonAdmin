...
@Component({
  ...
  templateUrl: './favourit-products.component.html',
  styleUrls: ['./favourit-products.component.scss'],
  standalone: true,
  imports: [CommonModule, TranslateModule, NgMaterialModule],
})
export class FavouritProductsComponent implements OnInit, AfterViewInit {
  displayedColumns: string[];
  dataSource: MatTableDataSource<FavoriteProductRow>;
  getOrders: FavoriteProductRow[] = [];
  ...
  ngOnInit(): void {
    ...
    this.authService.getFavourtieReport(object).subscribe((res: any) => {
      this.getOrders = res.data as FavoriteProductRow[];
      // console.log("Fef",this.getOrders)
      this.dataSource = new MatTableDataSource<FavoriteProductRow>(this.getOrders);
  ...
  callRolePermission() {
    if (sessionStorage.getItem('roleName') !== 'superAdmin') {
      ...
      const orderPermission = settingPermssion?.find((ele: Permission) => ele.area == 'favortie-product-reports')?.write == 1;
  ...
  exportIt() {
    ...
    this.exporter.exportTable(ExportType.XLSX, {
      fileName: `Favorite Product Report ${this.formattedDateTime}`,
    });
  }
}
